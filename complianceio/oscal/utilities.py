...
def use_org_name(text, org_name):
    ...
    if org_name is not None:
        text = text.replace("The organization", "The organization %s" % org_name)
        return text
    else:
        return text
...
def de_oscalize_control_id(control_id, catalog_key=None):
    ...
    return re.sub(
        r"^([A-Za-z][A-Za-z]-)([0-9]*)\.([0-9]*)$", r"\1\2(\3)", control_id
    ).upper()
...
def oscalize_control_id(cl_id):
    ...
    #   ac.1.001
    pattern = re.compile(
        "^[A-Za-z][A-Za-z]-[0-9() .]*$|^3\.[0-9]{1,2}\.\
            [0-9]{1,2}$|^[A-Za-z][A-Za-z][.][0-9][.][0-9]{1,3}"
    )
    if not pattern.match(cl_id):
        print("The given control could not be parsed given the Control ID structure")
        return cl_id
    ...
    cl_id = re.sub(
        r"^([A-Za-z][A-Za-z]-)([0-9]*)([ ]*)\(([0-9]*)\)$", r"\1\2.\4", cl_id
    )
...
def oscalize_catalog_key(catalogkey=None):
    ...
    if ".json" in catalogkey:
        catalogkey = (
            catalogkey.split("/")[-1].split("_catalog.json")[0].split(".json")[0]
        )
...
def get_control_statement_part(control_stmnt_id):
    """
    Parses part from control statement id ra-5_smt.a --> a
    """
...
def increment_element_name(component_name):
    ...
    if re.search("\((\d+)\)$", component_name):
        new_component_name = re.sub(
            "\((\d+)\)$",
            lambda m: " (" + str(int(m.groups()[0]) + 1) + ")",
            component_name,
        )
