...
from trestle.oscal.common import Link, Metadata, Part, Property, Role
...
def create_groups(p, impact_level, supplemental):
    """
    Parse the spreadsheet to get groups by family.
    """
    with open(Path.cwd().joinpath('complianceio', 'header31_map.json'), 'r') as f:
        c_map = json.load(f)
    wb = openpyxl.load_workbook(filename=p, data_only=True)
    if "Sheet1" not in wb.sheetnames:
        print(f"Sheetname 'Sheet1' not found in: {wb.sheetnames}")
        quit()
    ws = wb["Sheet1"]
    ...
    group = None
    controls = []
    for row in ws.iter_rows(min_row=2, max_col=19, max_row=1890):
        if row[0].value is not None:
            ...
            if baseline is None:
                if supplemental:
                    ...
                    continue
            if impact_level not in baseline:
                continue
            ...
            if control_text is not None:
                ...
                if "Supplemental" in baseline:
                    control_class = "ARS-3.1-Non-Mandatory"
                else:
                    control_class = "ARS-3.1-Mandatory"
                    props.append(
                        Property(
                            name='baseline',
...
parties = [
    {
        "id": "CMS-ISPG",
        "org": {
            "org-name": "Centers for Medicare & Medicaid Services Office of Information Technology (OIT)",
            "addresses": [
                {
                    "postal-address": [
                        "Information Security and Privacy Group 7500 Security Boulevard"
                    ],
                    "city": "Baltimore",
                    "state": "MD",
                    "postal-code": "21244-1850"
                }
            ],
            "email-addresses": [
                "ciso@cms.hhs.gov"
...
def main(title, source, ars_version, oscal_version, impact_level, supplemental):
    """
    Create an OSCAL Catalog from Excel (.xlsx) spreadsheet.
    Source: https://security.cms.gov/library/cms-acceptable-risk-safeguards-ars
    """
    p = Path(source)
    groups = create_groups(p, impact_level, supplemental)
    uuid = uuid4().urn
